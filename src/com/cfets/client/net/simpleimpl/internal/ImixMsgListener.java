...
import imix.field.TradeRequestID;
...
public class ImixMsgListener extends MessageCracker implements Listener {
	...
	@Override
	public void onMessage(ExecutionAcknowledgement message) throws FieldNotFound,
			UnsupportedMessageType, IncorrectTagValue {
		
		//1036			ExecAckStatus	0
		ExecAckStatus execAckStatus = new ExecAckStatus();
		if (message.isSetField(ExecAckStatus.FIELD)) {
			message.get(execAckStatus);
		}
		...
		//568 TradeRequestID	UDRID001	请求号
		TradeRequestID tradeRequestID = new TradeRequestID();
		if (message.isSetField(TradeRequestID.FIELD)) {
			message.get(tradeRequestID);
		}
		String requestId = tradeRequestID.getValue();
		char status = execAckStatus.getValue();
		String msg = msgField.getValue();
		
		logger.info("[FMS Server Response]--->: 请求号={},状态={}", requestId, status);
		logger.info("[FMS Server Response]--->: msg={}", msg);
	}
